...
the team's own \srcintent{design intake}: decentralization is the requirement, ``but done
...
drafts of this section carried as open parameters are fixed \srcintent{design intake}: a \emph{global} supply cap of \num{560000000}\flux{} across all chains, enforced by
...
$\hsnap$, $\hopen$, $\hclose$ & height & migration snapshot height; the opening of the one-directional window (retiring chains, March 2027) and its close at shutdown (August 2027)\\
...
constructor parameters only \srcintent{design intake}.
...
\srcintent{design intake}, so the construction below specifies immutability while
...
$\mathcal{C}_1 = \mathcal{C}_0 \cup \{\mathrm{SOL}\}$ following \srcintent{design intake};
...
\srcintent{design intake}, metered by a second leaky bucket
...
GUARDIAN $\mathcal{G}$ (any single member, 1-of-4) & pause mint for $\le \Delta_g$; cancel a
  queued admin action or upgrade \srcintent{design intake} &
...
ADMIN $(n_A,k_A)$, timelock $\Delta_A$ & change $\overline{M}_c$, $\varrho_c$, quorum sets, $\mathcal{G}$;
  ...
  \srcintent{design intake} & mint; transfer; freeze \\
\bottomrule
\end{tabular}
...
$\mathcal{G}$ & the same four, \textbf{1-of-4} & any single member pauses mint for $\le \Delta_g$ (\Cref{cor:pause}), or cancels a queued admin action or upgrade \srcintent{design intake} \\
...
\bottomrule
\end{tabular}
\caption{Decided bridge parameters \srcintent{design intake}. The quorum figures
...
$n$ transactions and nothing else \srcintent{design intake} \srcinferred. Nothing in
...
\srcintent{design intake}. Algorithm~\ref{alg:mint} is written for the whole-lock
...
own burn and blocks no other \srcintent{design intake}.}
\BlankLine
...
it \srcintent{design intake}; no later round chooses, so the choice is recorded here
as a recommendation rather than a decision \srcintent{design intake}. Under it H5
...
attributable to $g$, and the decided rule is 1-of-4 \srcintent{design intake}. The
...
before it executes \srcintent{design intake}; the call is attributable to $g$, as
the pause is.
...
\srcintent{design intake}. Under D3, replacement on EVM would be by migration:
...
range \srcintent{design intake}: were it a nonced burn, Algorithm~\ref{alg:release}
...
\srcintent{design intake}, with the single-member pause of \Cref{con:bridge}
...
named co-founders \srcintent{design intake}. Both sets sit well inside the P2SH
...
\srcintent{design intake}. Headroom at $t_0$ adds at most a further
...
window $\Delta_g$ \srcintent{design intake}. An adversary holding three of the four
...
same 3-of-4 \srcintent{design intake}. An adversary holding three keys is not
...
\srcintent{design intake}; without it the pause would defer the upgrade's effect
...
\srcintent{design intake}. Unanimity was considered and rejected for a
...
\srcintent{design intake} (\vision). The paper records this because it changes what
...
\srcintent{design intake} (\planned). Separately, the team states that code will
...
\srcintent{design intake}. The paper records that as the stated development
...
scenarios including chain reorganizations \srcintent{design intake}. The
...
node as secure and does nothing \srcprivate{\SAS{} watchdog} --- and under the team
...
a \textbf{global cap across all chains} \srcintent{design intake} (\planned).
...
\srcintent{design intake}. The global cap is a property of the reserve, not of the
...
\textbf{Settled at \num{250000000}\flux{}} \srcintent{design intake}
...
\srcprivate{\SAS{} API server}. A signature from that key therefore
...
itself a baked-in constant \srcprivate{\SAS{} disk encryption}. What
...
holder's \flux{} \srcintent{design intake}. That closes the construction question
...
\srcintent{design intake} --- with $\mathcal{G}$ the same four individuals but at a
...
worse than the custodial incumbent} \srcintent{design intake}. Taken seriously, it settles
...
for the autonomous bridge'' \srcintent{design intake}. Two things would make the position
...
\emph{relaunched} rather than kept \srcintent{design intake}; three new deployments would
...
\srcintent{design intake}. \textbf{Every other chain redeems through the
incumbent bridge} to the main chain during the redemption window, from $\hopen$ to $\hclose$, with unclaimed balances passing to
...
\srcintent{design intake}, and conflating them is what produced the
...
the forfeiture term; the second requires an act within the redemption window and therefore every
...
exchange partners and ecosystem integrators \srcintent{design intake}, which is not
...
(\Cref{tab:issuer-liability}) \srcintent{design intake}; the balance map
...
we did in the past'' \srcintent{design intake} (\planned). This is a stronger
...
\srcintent{design intake}. What matters for the migration argument is that the
...
\textbf{(ii) Announcement: $\hsnap$ is announced in advance} \srcintent{design intake} (\planned). This is the right choice and the paper does not hedge it: it lets holders
...
\srcintent{design intake}, and they replace the window and disposition
parameters this subsection previously carried as open. \planned

\begin{enumerate}
\item \textbf{Staged dates.} The new Ethereum and BNB Chain contracts go live first, on readiness and
      before the cut \srcintent{08-answers-round-2.md, round 34}; the retiring chains become
      one-directional at $\hopen$; the emission cut and \PNR activation land together at PA Depletion,
      $\hpa = \num{3787502}$ (2027-07-01) (\Cref{sec:pnr}, \Cref{sec:migration}); the retiring
      chains shut down at $\hclose$.
\item \textbf{Redemption window.} From $\hopen = \num{3450000}$ (2027-03-06) to
      $\hclose = \num{3880000}$ (2027-08-02) \srcintent{08-answers-round-2.md, round 36},
      \num{430000} L1 blocks, about \num{149} days at the \PoN target
      spacing, spanning the cut \srcinferred. During the window, redemption back to main-chain FLUX
      runs at 1:1 through the \textbf{existing bridge}.
\item \textbf{The redemption route is one-directional.} From $\hopen$ the existing bridge permits
      migrating \emph{to} the main chain, and mining entitlement stays claimable on the chain or on
      the main chain until shutdown; nothing can be swapped \emph{into} a retiring chain
      \srcintent{design intake}; \srcintent{08-answers-round-2.md, round 35}.
...
\bottomrule
\end{tabular}
\caption{Exposure to the redemption window and the unclaimed-balance term, by migration path.
...
\noindent So the amount exposed to the redemption window and the unclaimed-balance term is
...
within the window is unknown, and this paper does not estimate it}: there is no holder-count or
...
mitigation.} First, the incumbent's custody model is not retired at $\hpa$ but at window close, $\hclose$,
about a month later --- and it is already reduced to a one-directional exit from $\hopen$, four
months before the cut. For that interval the bound of \Cref{prop:worst-case} applies to the new
...
path from the time its contracts go live; it is not a claim about the window. \textbf{Second, and this materially narrows
the first: the route is one-directional} \srcintent{design intake}. The incumbent's
...
\textbf{It will not be deployed} \srcintent{design intake} (\planned), and the
...
$\hsnap$ \srcintent{design intake}. Holders would be reached through per-asset redemption statistics
...
\srcintent{design intake} --- the bulk of five EVM contracts' \num{440000000}-unit pre-mint each,
...
recorded as decided \srcintent{design intake}.}
...
and BSC \srcintent{design intake}. Base is not BSC --- it is an Ethereum L2, and they differ
...
re-addition is contemplated} \srcintent{design intake} --- this closes a question
...
\item \textbf{Carry the snapshot} on ETH and BSC (Algorithm~\ref{alg:carry}), with exchange partners
      ...
      \srcintent{design intake}. A custodian must know before $\hsnap$
...
\item \textbf{Foundation immobilises its legacy reserves} on the retiring chains, by transfer to
      the published per-chain dead addresses \srcintent{design intake}.
...
\item \textbf{Close} the window at $\hclose = \num{3880000}$ (2027-08-02); the
...
\noindent Steps 3--7 are staged \srcintent{08-answers-round-2.md, round 34}: the Ethereum and BNB Chain
carry and public lock-to-mint first, on readiness and before $\hpa$; holder-initiated migration on
the retiring assets from $\hopen$; the close at $\hclose$. Steps 1 and 2 must complete before the
first of these, and step 2 is not complete: $\Lambda_c$ is measured
...
\srcintent{design intake}, so public mint opens under named signers and the attestation link
...
\srcintent{design intake}, the $T_{\mathrm{react}}$ target in round 18.

\begin{table}[H]
\centering\footnotesize
\caption{Bridge parameters. Proposed by this paper and \textbf{confirmed by the author}
\srcintent{design intake}, the $T_{\mathrm{react}}$ target in round 18; the two
...
contract constant \srcintent{design intake} \\
...
the migration window is about \num{149} days, so the \emph{mean} redemption rate over the window is
$\num{6226900.80}/149 \approx \num{41791}\flux$ per day \srcinferred. Redemption will not be uniform
...
week. \num{500000}\flux{} is roughly twelve days at the mean, which absorbs a first-week surge of
several times the average without a cold-storage round trip, and the \num{200000}\flux{} top-up
trigger leaves about five days of cover at the point the top-up is initiated. Both figures should be
...
\srcintent{design intake}. Under D3 the Nomad mechanism is unavailable; under the
