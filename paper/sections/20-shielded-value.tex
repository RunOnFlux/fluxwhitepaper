...
permanently'' \srcintent{05-privacy-pq.md}. Those statements are accurately reported and were
accurate when made. They have been superseded: the team has decided to \textbf{retire both shielded
pools} \srcintent{08-answers-round-2.md, round 10} (\planned). The transparent layer stays; the
...
(\S\ref{sec:shielded-retirement}, \srcintent{08-answers-round-2.md, round 10}). An earlier round of
...
matures \srcintent{05-privacy-pq.md} (\vision; after the retirement decision of
...
for \srcintent{05-privacy-pq.md} (\vision). That argument stands on its own regardless of the
...
\textbf{Both shielded pools are to be retired} \srcintent{08-answers-round-2.md, round 10}
...
\srcintent{08-answers-round-2.md, round 9} --- and, presented with the analysis below, took it.
...
\begin{itemize}
  \item \textbf{Announcement at $\hpa = \num{3466630}$}, the height at which the parallel-asset
  consolidation is already scheduled to take effect (\S\ref{sec:pa-sequencing}). One consolidation message covering
  both is better than two, and the holder sets barely overlap.
  \item \textbf{Sunset at $\hshield = \num{4517830}$}, i.e. $\hpa + \num{1051200}$ blocks
  $\approx$ 12 months at the \SI{30}{\second} target. After this height, transactions carrying
  \texttt{vShieldedSpend} or \texttt{vJoinSplit} are invalid; notes not swept are permanently
  unspendable. Twelve months is deliberately longer than the six-month parallel-asset window
  (\S\ref{sec:bridging-migration}): that window ends in forfeiture to a named recipient, whereas this
  one ends in destruction, which warrants more notice for less value.
...
\srcintent{08-answers-round-2.md, round 11}: announcement at $\hpa = \num{3466630}$ and sunset at
$\hshield = \num{4517830}$. Activating ZIP-209 turnstile enforcement with Flux-correct constants for
the window remains this paper's recommendation, not a confirmed decision: round 11 confirmed the
numbers, and a switch is not a number \srcintent{08-answers-round-2.md, round 18}. The retirement
...
\srcintent{05-privacy-pq.md}; after retirement the honest description is that Flux settles
