...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[
    ...
    %% 10mm gaps were narrower than the edge labels they had to hold, so
    %% "A record" and "proxied" ran into the adjacent box borders.
    \node[found, right=17mm of client] (dns) {DNS\\[-1pt]{\scriptsize Cloudflare \emph{or} \PDNS}};
    \node[found, right=17mm of dns]    (ha)  {HAProxy\\[-1pt]{\scriptsize on the elected \FDM host}};
    \node[flux,  right=17mm of ha]     (app) {application\\[-1pt]{\scriptsize container IP:port}};

    %% above=0.9mm rather than a bare `above': the labels were sitting on the arrows.
    \draw[arr] (client) -- node[lbl, above=0.9mm] {name} (dns);
    \draw[arr] (dns) -- node[lbl, above=0.9mm] {\texttt{A} record} (ha);
    \draw[arr] (ha)  -- node[lbl, above=0.9mm] {proxied} (app);
    ...
    \node[lbl, above=4mm of ha] (tls) {TLS termination, Host-header ACL,\\[-1pt]per-server health check};
...
periodically against the \texttt{runonflux.io} domain \srcintent{design intake}. This is less of a concentration than it first reads. Behind any certificate there must be
