...
funding transaction are merged. That is a serious finding about the proposal. It is also, and
simultaneously, an improvement on what is deployed, where the corresponding number is one, the
decision carries no signature, and no node can verify that it was ever made. Both statements are true
and the section makes both.

Two qualifications belong here rather than in a footnote, because without them the finding reads as
an accident and it is not one. First, \textbf{the identities behind that concentration are largely
known}: the largest is a pooled-staking operator running nodes on behalf of many individual
depositors, so its weight aggregates people who did not individually choose how it votes, and the
next largest are the Foundation and team members whose holdings are publicly disclosed
\srcintent{design intake}. That is a different risk from four anonymous whales and should not be
described as though it were the same one --- though it is not therefore benign, since pooled weight
is still weight and disclosure is not dispersion.

Second, \textbf{the centralised moderation deployed today is a deliberate capability, not an
oversight.} A network that serves applications under its own domains needs some party able to remove
a phishing site impersonating \texttt{runonflux.io}, or content that exposes operators to legal
jeopardy in their own jurisdictions, and today that party is the team acting through \FDM{} and the
policy channel of \Cref{sec:networking}. The specified quorum exists precisely to move that authority
from a team to the network, and the concentration measured above is the distance still to travel
rather than a property of the destination \srcintent{design intake}. What the measurement establishes
is that the move does not complete on activation: at the present distribution the quorum relocates
the capability without yet dispersing it.
...
\srcintent{design intake} (\planned). That key set is not a
...
commitment; it is a design-intake item \srcintent{design intake} that must be
...
mechanism forces either reading; the taxonomy does, and its scope has been ruled on
(\Cref{rem:content-veto}).

\begin{remark}[The scope is behavioural harm, and the deciding party is the operators]
...
--- harm to third parties, to operators, to the network's own infrastructure --- so as to reconcile
the quorum with the roadmap's published ``behaviour, not content'' principle. \textbf{The answer is
that the scope is behavioural harm} \srcintent{design intake} (\planned). The roadmap's principle
stands, and the quorum is an enforcement path for it rather than a departure from it. An earlier
draft of this remark recorded the opposite and is superseded.

Two consequences, and the second is the one that decides how the whole section should be read.

\begin{enumerate}
  \item \textbf{The taxonomy carries the limit, so concentration is bounded rather than unbounded.}
  A behavioural taxonomy limits the damage a captured quorum can do, because the categories
  themselves resist misuse: a report has to allege harm of a kind that other operators can recognise
  and that the reported application's own behaviour either exhibits or does not. The measurement in
  \Cref{sec:23:measurement} --- four owner identities reaching the \SI{25}{\percent} threshold ---
  remains the mechanism's most important number, but it is now a bound on a bounded power rather
  than on an unbounded one, and \Cref{sec:23:priorart}'s comparison with IPFS, Filecoin and Arweave
  holds on intent as well as on the facts.
  \item \textbf{The deciding party is the node operators, and that is not a power the protocol
  grants them.} \srcintent{design intake} The people running the machines can already refuse to run
  a workload, in exactly the sense that miners in a proof-of-work system can already refuse to
  include a transaction; nothing a specification says removes that. What the quorum changes is not
  \emph{whether} operators can collectively remove an application but \emph{how}: today the decision
  is one Foundation pull request that every node obeys without checking
  (\Cref{prop:23:kappa-today}); under the quorum it is a signed, weighted, on-chain vote that names
  every participating key and is verifiable by anyone
  (\Cref{thm:23:verifiable}). Read that way the mechanism is a transfer of an existing capability
  from a publisher to the operators, made legible --- and the honest expectation, which the paper
  states as an expectation and not as a guarantee, is that operators do not spend collateral-weighted
  votes on applications that are not harming anyone.
\end{enumerate}

The residual risk is not removed by either point and is not softened here: a collateral-weighted
electorate concentrated enough that four identities clear the threshold can, in principle, agree to
call something harmful that is not. \Cref{sec:23:negative} quantifies exactly that and does not
argue it away. What has changed is the size of the prize: under a behavioural taxonomy such a
coalition is misusing a mechanism against its stated categories, in public, with its keys on the
record --- rather than exercising a power the paper had granted it in advance.

What remains open is the enumeration: which categories exist, so that a report is a well-formed
object with a fixed domain rather than free text. That is a specification task, not a policy
question, and \Cref{sec:23:mechanism} states the requirement it must satisfy.
\end{remark}
...
paper; the settled inputs are the design intake \srcintent{design intake}, the
on-chain construction supplied by the team
\srcintent{design intake} and the measured node set
...
operates nodes \srcintent{design intake}.
...
(\Cref{def:23:ban}) \srcintent{design intake}. Its confirmation
...
\begin{definition}[Voting delegation, \planned]
\label{def:23:delegation}
An identity $i \in I(\hgt)$ may publish a \emph{delegation}: a \fluxd{} transaction of type
$\mathsf{MOD\_DELEGATE}$ carrying $(i,\ \mathrm{pk}_{\mathrm{vote}},\ \hgt_d)$ and signed by the
owner key $i$, authorising $\mathrm{pk}_{\mathrm{vote}}$ to sign $\mathsf{MOD\_VOTE}$ transactions on
$i$'s behalf from height $\hgt_d$ onward. A vote is then valid if it is signed either by $i$ or by
the delegate current for $i$ at $\hgt_0$. A delegation is \emph{revoked} by a later delegation from
the same owner key naming a different delegate or the null key; revocation takes effect at its own
confirmation height and never retroactively. Three properties are required, and all three hold by
construction:
\begin{enumerate}
  \item \textbf{Weight does not move.} $\vweight{i}$ is a function of $i$'s collateral
  (\Cref{def:23:electorate}) and is unchanged by delegation. A delegate holding $d$ delegations casts
  $d$ separate votes carrying $d$ separate owners' weights; it never accumulates weight of its own,
  and it is counted in the identity chamber of \Cref{def:23:twochamber} once per delegating owner and
  never once for itself.
  \item \textbf{$\kappa$ does not move.} \Cref{thm:23:collusion} counts owner identities whose weight
  sums past $\vthresh$. Delegation changes who holds a signing key, not whose weight it carries, so
  $\kappa(0.25) = 4$ is unchanged.
  \item \textbf{The record does not degrade.} The chain names both the delegate that signed and the
  owner whose weight was cast, so \Cref{thm:23:verifiable} holds verbatim with one extra lookup.
\end{enumerate}
\end{definition}

\decided{Opt-in revocable delegation is adopted \srcintent{design intake}. The reason is liveness and
it is not a close call. Collateral keys are cold by design --- that is the point of them --- and a
rule asking \num{21} tenured identities plus \SI{25}{\percent} of all weight to produce owner-key
signatures inside \SI{24}{\hour} makes expiry the expected outcome of an honest report rather than an
edge case. A mechanism whose safety argument depends on the dispersed majority being able to
participate cannot price participation at ``retrieve your cold collateral key today''. The cost is
real and is stated rather than netted: delegation concentrates signing authority in delegates by
construction, which is the documented trajectory of every delegated token-vote system, and it makes
delegates the party a coalition would rent or coerce. The paper's position is that this trades a
certainty against a tendency --- certain non-liveness against a tendency the three properties above
confine to \emph{signing} rather than \emph{weight} --- and that publishing the delegation graph at
every reduction height, alongside $\kappa(\vthresh)$ and the top-$N$ shares
(\Cref{sec:23:endgame}), is what keeps the tendency observable.}
...
path at all, and an underpaid registration is silently dropped \srcdoc{orchestration extraction} — so the
...
\srcintent{design intake}. What it buys is stated plainly:
...
\paragraph{Recommendation: an explicit network-wide cap, well below $\lvert N(\hgt)\rvert$.} The
per-identity capacity rule of \Cref{def:23:capacity} is the right anti-Sybil bound and nothing here
displaces it. But it was never chosen as a block-space bound, and as a block-space bound it is far too
loose: it permits a quarter of the chain's entire throughput to be consumed by transactions that pay
nothing. Nothing about the mechanism's purpose requires \num{6489} simultaneous reports. A network-wide
cap of order \num{64} concurrently open reports, with further reports queued in confirmation order
rather than rejected, bounds the worst case to under \SI{5}{\kilo\byte} per block --- three orders of
magnitude below the block limit --- costs nothing in the honest case, since the deployed registry
holds \num{1195} applications and no plausible incident implicates more than a handful at once, and
composes with \Cref{def:23:capacity} rather than replacing it: an identity still cannot open more
reports than it operates nodes, and now the network cannot be made to carry more than it can afford.
The cap belongs with the settled parameters rather than in the open list, and \Cref{tab:23-recommended}
should carry it.
...
\srcintent{design intake}, and the reason given is worth recording because
...
\begin{property}[A ban is final, and what it is final \emph{about}, \planned]
...
application \srcintent{design intake}.

\textbf{The scope of that finality is narrower than the word suggests, and stating it precisely
answers the strongest objection to this mechanism.} A ban removes one application from the network,
permanently and irreversibly. It is \emph{not} a forward prohibition on the party that deployed it:
the owner may register another application in the next block, at the ordinary price, with no
residual mark against their key \srcintent{design intake}. Nor does it retire the application
\emph{name}: the registry entry is released, and the name is available again --- to any other owner
immediately, and, under the settled $(\mathsf{app\_name}, \mathsf{owner})$ keying, to every party
except the one whose deployment was removed. So the object made permanent is a single deployment. It
is not an identity, not a namespace reservation, and not an exclusion from the network.

That distinction is the difference between a removal power and a censorship power, and it is why the
absence of a reinstatement path is defensible where an absence of recourse would not be. A wrongly
banned tenant does not appeal; they redeploy, in minutes, for the price of one transaction. What they
lose is the removed instance's reachability --- \FDM{} and \PDNS{} names are keyed per application
name --- their unearned prepaid balance, and their continuity. What they do not lose is the ability
to operate on \Flux{}. The paper does not present that as costless, and \Cref{sec:23:attacks} keeps
the wrongful-eviction case in the failure surface; it presents it as the reason a monotone ban set is
compatible with a network that has no appeal mechanism and does not need one. Both absences are
...
One consequence of the keying is worth stating precisely rather than absorbing silently. Under the
settled $(\mathsf{app\_name}, \mathsf{owner})$ scope, the pair is what enters the ban set, so the
banned owner cannot re-take that particular name --- which is the whole of what ``final'' costs them
--- while any other owner can, and while they themselves can register anything else. Had
$\mathsf{app\_hash}$ instead resolved to the application's \emph{content}, a specification or image
hash, the finality would have bitten differently and worse: an otherwise identical redeployment by
\emph{anyone} would carry the same hash and be immediately banned with no fresh report and no fresh
vote, which turns a decision about one deployer into a network-wide prohibition on a piece of
software. That is the substantive reason the identity reading was chosen over the content reading,
and it is recorded here because the two readings differ in exactly the property this subsection is
about.
\end{property}
...
continuously \srcintent{design intake}. Combined with
...
be more decentralized'' \srcintent{design intake} (\planned).
...
incentivised to be honest \srcintent{design intake}. The argument is correct in
...
applications in the \PNR{} demand baseline \srcintent{mechanism specification} — the registry returns
...
final and the remedy is redeployment (\Cref{op:23:reinstatement}) --- a remedy that is available
immediately and costs one transaction, because finality attaches to a deployment and not to the
deployer.
Concentration.

\paragraph{Opt-in delegation (adopted, \Cref{def:23:delegation}).} \emph{Fixes:} liveness, which is
the mechanism's scarcest resource. Turnout rises without operators following every report, and a
higher, safer $\vthresh$ becomes reachable later. \emph{Costs:} delegation concentrates
\emph{signing authority} in delegates by construction --- the documented trajectory of every
delegated token-vote system --- and makes delegates the party a coalition would rent or coerce.
\emph{Does not fix:} concentration of weight; it does not worsen it either, because
\Cref{def:23:delegation} leaves $\vweight{\cdot}$ and $\kappa$ at the owner. The decisive argument
for adopting it is that the alternative is not a safer mechanism but an inert one: owner keys are
cold by design, and a \SI{24}{\hour} owner-key signing requirement makes expiry the expected outcome
of an honest report.
...
chosen: the per-application cooldown is zero, the per-identity bound on open reports is $X_i(\hgt_0)$
(\Cref{def:23:capacity}), and reporter eligibility is $I(\hgt_0)$. The \emph{network-wide} cap is a
separate parameter and is no longer left at $\lvert N(\hgt)\rvert$: see \Cref{tab:23-recommended},
where it is set to \num{64} with queueing, for the block-space reason given in
\Cref{cor:23:capacity}'s discussion.}
...
transaction, and it resets \texttt{added\_height} --- an inference the earlier draft flagged as
load-bearing and unverified, and which is \textbf{now verified against source}. \fluxd{} keys its
node cache on the collateral outpoint, so a respend produces a different key and therefore a
different record \shipped
\srccode{fluxd/\allowbreak{}src/\allowbreak{}fluxnode/\allowbreak{}fluxnode.h}{148--160};
\texttt{Check\allowbreak{}New\allowbreak{}Start\allowbreak{}Tx} rejects a second start against an outpoint
already tracked, so the old record cannot be carried over \shipped
\srccode{fluxd/\allowbreak{}src/\allowbreak{}fluxnode/\allowbreak{}fluxnode.cpp}{445--476}; and
\texttt{Add\allowbreak{}New\allowbreak{}Start} writes \texttt{nAddedBlockHeight = p\_nHeight} together
with \texttt{nLastPaidHeight = 0} on every admission \shipped
\srccode{fluxd/\allowbreak{}src/\allowbreak{}fluxnode/\allowbreak{}fluxnode.cpp}{326--341}. Nor is there a
transfer path that would avoid this: the node transaction family is exactly
$\{\textsc{start},\ \textsc{confirm}\}$ \shipped
\srccode{fluxd/\allowbreak{}src/\allowbreak{}primitives/\allowbreak{}transaction.h}{618--619}, with no
owner-key update type. Tenure therefore resets, and so does rotation position --- the re-addressed
node re-enters the queue as never-paid, keyed by confirmation height \shipped
\srccode{fluxd/\allowbreak{}src/\allowbreak{}fluxnode/\allowbreak{}fluxnode.h}{313--328} --- one missed
Stratus rotation being \num{9}\flux, about \SI{0.155}{\percent} of that node's annual subsidy income.
\textbf{The tenure gate does the work the construction claims it does}, and $m$ and $T$ can be set on
that basis rather than provisionally.
...
\paragraph{What $T = \SI{30}{\day}$ does not buy, stated because the table above invites the wrong
reading.} The last column of the tenure table moves by \num{0.1} percentage points between
$T = \SI{7}{\day}$ and $T = \SI{30}{\day}$ (\SI{24.05}{\percent} to \SI{23.95}{\percent}): the large
keys' nodes are overwhelmingly older than thirty days, so the settled $T$ excludes essentially none
of them. Anyone reading the gate as a filter on the concentrated weight should stop --- it is not
one, and \Cref{prop:23:twochamber} is explicit that $\kappa(0.25) = 4$ is unchanged.

What $T$ actually sets is \emph{lead time}: how far in advance a coalition must position the
identities it will use, and therefore how much warning the network gets. Thirty days is a short
planning horizon for an adversary who is planning at all. Ninety days is the smallest value in the
domain that is awkward to arrange quietly, and it costs \SI{1.6}{\percent} more required
participation and disenfranchises the median node --- a real cost against a mechanism whose scarcest
resource is turnout. The paper records the trade squarely rather than recommending its way out of it,
and notes the one thing that holds at every $T$: because re-addressing is a chain event, a burst of it
is datable whether or not the gate would have caught the resulting identities. \textbf{The warning is
the durable value; $T$ only decides how early it arrives.} If $T = \SI{30}{\day}$ is kept, it should
be kept for that reason and not described as a barrier to the identities the measurement names.
...
introduce penalties for malicious submissions \srcintent{design intake}. This is
...
verification of what it serves as the latest release \shipped~\srcprivate{release API}; the
Foundation-held application owner keys used for auto-renewal \shipped~\srcprivate{renewal service};
and a
...
That step is worth pricing, because the gap it closes is the largest single one in this section and
it is not a design problem. As deployed, $\kappa_{\mathrm{deployed}} = 1$ with no authentication
means a compromised organisation account, or a party able to interpose on the raw-content fetch,
removes any application fleet-wide within the \SIrange{7}{13}{\hour} bound of
\Cref{prop:23:kappa-today}, and no node can tell that from a legitimate merge. The quorum specified
in this section is a network upgrade with two new transaction types, new chain-derived state and a
new coinbase rule; document signing is a week of work in a repository that has already written down
what to do. \textbf{As with the attested-execution findings
(\S\ref{sec:14-disposition}), the team's disposition is to close the cheap items ahead of
publication} \srcintent{design intake} --- document signing here, and the deployed-service items
listed above that are security tickets rather than governance design: \texttt{fluxstorage} holding
environment variables and commands unencrypted with no write-side protection, a \texttt{fluxteam}
privilege standing above a node owner's own administrative rights on that owner's machine, and the
release API's missing verification of what it serves. The first two are cited above by file and line
because those repositories are public; the third is not, for the reason
\S\ref{sec:14-disposition} gives. None of the three requires a decision from anyone; they require a
sprint. The \specv{9} fork's location is in the same class: moving the branch into the
organisation's repository costs nothing and removes a fact the paper would otherwise have to keep
reporting.
...
paper and \textbf{confirmed by the author} \srcintent{design intake}; the reasoning
...
\textbf{confirmed by the author} \srcintent{design intake}. \planned,
...
(\Cref{sec:pnr}); one tenure constant, two mechanisms. Buys \SI{30}{\day} of adversary lead time, not
exclusion of the concentrated keys (\Cref{def:23:twochamber}) \\
network-wide open reports & \textbf{\num{64}}, queued & the per-identity rule bounds Sybils, not block
space; $\lvert N(\hgt)\rvert$ concurrent reports is a quarter of chain capacity in fee-less
transactions (\Cref{sec:23:enforcement}) \\
voting delegation & \textbf{adopted}, revocable by supersession & cold owner keys cannot answer a
\SI{24}{\hour} window; weight and $\kappa$ stay at the owner (\Cref{def:23:delegation}) \\
\bottomrule
\end{tabular}
\end{table}
...
— is on the table at all. The scope is confined to behavioural harm
(\Cref{rem:content-veto}), which is what keeps that removal an answer to the storing-versus-executing
difference rather than a general veto over what may be said. It is also why concentration matters
more here than there: a denylist a node may ignore is a recommendation; a ban carried in the chain
that every node must honour is a power, whatever its stated scope.
...
own principle, and which the quorum honours, since the taxonomy is confined to behaviour
(\Cref{rem:content-veto}).
...
host this''. The quorum is the latter, and the paper presents it as a deliberate departure from the
Filecoin and Arweave position on \emph{scope of enforcement} --- justified by the difference between
storing and executing --- while agreeing with them on \emph{scope of subject matter}. Those are two
different axes and conflating them is what makes this comparison hard to read; the paper separates
them here so that a reader can disagree with one without inheriting the other.
...
Two further items are recorded here rather than marked in place, because they are not parameters of
this mechanism. (A third, whether opt-in delegation is adopted, is now decided: it is, with
revocation by supersession --- \Cref{def:23:delegation}.) Whether \FDM{} and \PDNS{} honour bans by dropping routes at
...
Three items cannot be set by parameter choice. Two were \emph{verifications} in \fluxd{} and both
are now closed against source: whether the emergency-block path is subject to the coinbase rules the
ban memo would rely on (\Cref{rem:emergency-coinbase}, favourably), and whether re-addressing resets
\texttt{added\_height}, which decides whether the tenure gate of \Cref{def:23:twochamber} does any
work at all (\Cref{def:23:twochamber}: it does). The third cannot be resolved at all from public
data:
