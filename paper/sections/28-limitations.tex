...
Every entry below is recorded in \srcdoc{reconciliation record} as a claim the drafting process considered and
...
measurable from public data, rather than estimating it (\srcdoc{reconciliation record}). Second, operator
...
key, as an open problem below (\srcdoc{reconciliation record}).

\paragraph{Not every cited repository is one the reader can open.} This is the most important
limitation on the paper's own method, and it is stated first among the caveats about citation rather
than buried among them. The paper cites source by file and line throughout, and for the majority of
its citations --- \fluxd{}, \FluxOS{}, the network-policy channel, \FDM{}, the node front end, the
storage service --- those repositories are public and any reader can check the line. For a minority
they cannot: the benchmark client, the \SAS{} and its enrollment tooling, the \ArcaneOS{} image
build, the HSM ceremony tooling, the release API, the auto-renewal service and the reachability
fleet's inventory are private repositories. Claims cited to those rest on the authors' reading, and
a reader has no way to audit them.

Two consequences are worth being explicit about. First, the paper's central promise --- that the
described behaviour could be reimplemented or audited from what is cited --- holds fully for the
public components and only conditionally for the private ones; where a load-bearing claim could be
re-anchored to public source it has been, and \S\ref{sec:consensus}'s benchmark-key finding is
cited to \fluxd{}'s own consensus constants for exactly that reason
\srccode{fluxd/\allowbreak{}src/\allowbreak{}chainparams.cpp}{233--238}. Second, two of these components are
private \emph{by decision} rather than by omission: the benchmark client and the \SAS{} remain
closed until a mechanism exists that verifies node trustworthiness in a way that cannot be faked
\srcintent{design intake}. That decision is the reason \S\ref{sec:14-disposition} states one class of
finding as properties without coordinates, and a reader who wants to hold this paper to its own
standard should know that the redaction is deliberate, bounded to that class, and explained where it
is applied rather than exercised silently.
...
release-notes directory listing (§20; \srcdoc{reconciliation record}). The
...
measured from a block explorer with a retrieval date (\srcdoc{reconciliation record}). The Kadena parallel
...
is replenished (\srcdoc{reconciliation record}).
...
found unsupportable; each is recorded in \srcdoc{reconciliation record} with the claim, why it fails, and what
...
properties below (\srcdoc{reconciliation record}). The remaining seven are stated once, here, and nowhere else in
...
(\srcdoc{reconciliation record}); that supply reaches a terminal value is false, so §5 gives the supply
function as unbounded with a vanishing rate and states its tail explicitly
(\srcdoc{reconciliation record}); that \Flux implements a proof-of-useful-work mechanism is false — the
...
(\srcdoc{reconciliation record}); that GPUs are hot-attachable is unsupported by any code in the four
GPU-adjacent repositories, so §15 states whole-device passthrough as the deployed mechanism and tags
hot-attach \vision{} (\srcdoc{reconciliation record}); that shielded supply is fully publicly verifiable is
...
(\srcdoc{reconciliation record}); that application data survives a payment lapse is unsupported because no
...
(\srcdoc{reconciliation record}); and that the network compensates a tenant for node failure is
unsupported because no refund or SLA-credit mechanism exists anywhere, so §16 states that the remedy on
offer is replacement, not refund (\srcdoc{reconciliation record}).
...
\item Benchmark results are not cryptographically bound to the operator or the host that produced
  them (§4). \shipped; \srccode{fluxd/\allowbreak{}src/\allowbreak{}chainparams.cpp}{233--238}.
\item Attestations are not bound to the machine that produced them, and no tenant-facing verification
  path exists (§14). \shipped; \srcprivate{\SAS{} API server}.
...
\item A \Flux light client is not constructible on an external chain under the current header format,
  ...
  \srccode{fluxd/\allowbreak{}src/\allowbreak{}pow.cpp}{284--290}; \srcintent{mechanism specification}.
...
build that merely has not happened yet \srcdoc{reconciliation record}. Each is stated with what would
...
\srcdoc{reconciliation record}. The roadmap states the realistic threat as store-now-decrypt-later
...
\srcdoc{reconciliation record}. Value sitting in ordinary transparent or shielded
...
spending cap \srcdoc{reconciliation record}. The deployable answer available today — fund a
...
\srcdoc{reconciliation record}. Two further constraints, drawn from the formal §21 design,
...
\srcdoc{open-decisions record, item 11}. No reweighting, threshold change, or remedy within the
...
\srcdoc{open-decisions record} lists roughly eighty parameters that were open when drafting
...
\srcintent{design intake} have since settled most of them, each in the section that uses the
...
the resulting height, deliberately decoupled from $\hstar$ \srcintent{design intake}
...
\srcintent{design intake} and not returned to since & the author (editorial framing
decision) & §17, §18, §19 \\
\addlinespace
...
\srcintent{design intake} & the author & §5, §20, §21 \\
\addlinespace
...
is settled \srcintent{design intake} and the enumeration is not measured in this
corpus & Foundation and exchange partners & §22 \\
\addlinespace
Moderation category taxonomy: the enumeration of reportable categories, whose scope is ruled
behavioural harm \srcintent{design intake} and whose members are undecided
