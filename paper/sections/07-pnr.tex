...
\srcintent{design intake}. The purpose of this section is to state that
...
makes operator income independent of which applications a node runs. That second point is a
statement about the eligibility gate rather than about willingness to work, because placement is the
network's choice and not the operator's (\S\ref{sec:pnr:hosting}).
...
nodes \PoN has already selected \planned \srcintent{design intake}. \PNR consumes \PoN's
...
\begin{enumerate}
  ...
  \item[\textbf{A3}] \emph{Emergency block production exists.} A hardcoded 2-of-4 key set can
    ...
    mechanism must be designed against, not a hypothetical. It is also, already, a public fact: the
    key count, the threshold and the absent gating are all readable in an open-source repository and
    every emergency block ever produced is on the chain, so stating it here discloses nothing that was
    not disclosed by shipping it. What publication does change is that the capability becomes
    \emph{legible}, and the paper's position is that the right response is to close it rather than to
    leave it undescribed: time or stall gating in \fluxd{}, and the move to 3-of-4 over a freshly
    generated key set that \S\ref{sec:governance} records as the team's intent, are both cheap
    relative to the design work S1 below has to do to contain it.
  ...
  \item[\textbf{A5}] \emph{Consensus can recognise only what it can parse --- and a payment to a
    ...
    rule; the obligation to pay, and the price, are policy} \srcdoc{reconciliation record}.
\end{enumerate}
\end{assumption}
...
\srcdoc{scripts/07-pnr-settlement.py, run 2026-09-07}; the one-node difference changes nothing at
...
receipt total \srcintent{design intake}; the census over the same window returned
...
$\hpa = 3{,}787{,}502$, $\rampstep = 0.05$ and $\nodesharecap = 0.50$
\srcintent{design intake}. Write $r(\hgt) \defeq 100\,\nodeshare(\hgt) \in \mathbb{Z}$.
...
\begin{description}
  ...
  \item[PS4 (consensus split).] On connecting the block containing the payment at height $\hgt$,
    ...
    The magnitude belongs at the point of definition and not only in \S\ref{sec:pnr:related}: at
    activation this rule would send the Foundation \textbf{four times what it sends operators} from
    every payment, and $\nodeshare$ does not reach $\nodesharecap = 0.50$ until the reduction of
    October 2032. What PS4 changes is not, at first, how much --- today \SI{100}{\percent} of the
    same money reaches the same address by policy --- but \emph{who can alter it}, and by what
    procedure. That is the honest form of the claim, and a reader who wants the fraction to be
    different should read \S\ref{sec:pnr:related}, where it is compared against Akash's, rather than
    infer that consensus enforcement is itself the concession.
\end{description}
...
revealed-amount output \srcintent{design intake}, which would have made PS4
...
\begin{enumerate}
  ...
  \item[\textbf{E3}] \emph{The coinbase would gain no output.} The Foundation share would be added
    ...
    The team's constraint that the coinbase must not grow \srcintent{design intake}
...
\srcintent{design intake}, and after activation the same quantity would be measured
...
\paragraph{The aggregation, now read from the source rather than stated as an interface.} An earlier
draft carried the units, rounding and multipliers of $\price$ as unestablished. They are established
here, because a \flux-denominated consensus split over a quantity nobody has pinned down is not a
specification. In \FluxOS's \texttt{app\allowbreak{}Price\allowbreak{}Per\allowbreak{}Month}, the
applicable row is the last table entry with $\text{height} < \hgt$, and the rates carry implicit
units: \texttt{cpu} is priced \emph{per \num{0.1} core} (the core count is multiplied by ten),
\texttt{ram} \emph{per \SI{100}{\mega\byte}} (divided by one hundred) and \texttt{hdd} \emph{per
\SI{1}{\giga\byte}}, summed across all components of a \specv{4}-or-later composition \shipped
\srccode{flux/\allowbreak{}ZelBack/\allowbreak{}src/\allowbreak{}services/\allowbreak{}utils/\allowbreak{}appUtilities.js}{31--33,91--115}.
The three surcharges are added once each where they apply: \texttt{scope} for an enterprise or
node-targeted application, \texttt{staticip} for a static-IP request, and \texttt{port} \emph{per
enterprise port} \shipped
\srccode{flux/\allowbreak{}ZelBack/\allowbreak{}src/\allowbreak{}services/\allowbreak{}utils/\allowbreak{}appUtilities.js}{116--122}.

Two details matter to \S\ref{sec:pnr:crossover} and are easy to get wrong. First, \textbf{the table
prices three instances}: the summed total is divided by three to obtain the per-instance price, and
instances beyond the first are added on top, with a \num{0.5}\flux{} floor per extra instance where
the base price is below that and more than two extras are requested \shipped
\srccode{flux/\allowbreak{}ZelBack/\allowbreak{}src/\allowbreak{}services/\allowbreak{}utils/\allowbreak{}appUtilities.js}{124--134}.
Second, rounding is a \emph{ceiling} to \num{0.01}\flux{}, taken twice --- once on the per-instance
price and again after the duration multiplier $\dur/L^{\ast}(\hgt)$ is applied \shipped
\srccode{flux/\allowbreak{}ZelBack/\allowbreak{}src/\allowbreak{}services/\allowbreak{}utils/\allowbreak{}appSpecHelpers.js}{55--63}.
No geolocation multiplier exists in this path. A marketplace multiplier and duration discounts of
\num{3}/\num{6}/\SI{12}{\percent} do exist, but only in the USD-denominated \emph{quote}
(\S\ref{sec:pnr:openparams}, O9), never in the on-chain \flux{} price that \FluxOS{} validates a
payment against --- which is the quantity PS4 would divide. O3 is closed and this marker with it.
...
address-and-memo path without invalidating it \planned \srcdoc{reconciliation record}.
...
above, with every validator recomputing rather than trusting it \srcdoc{reconciliation record}. Three
...
\srcdoc{mechanism specification}. \PNR would therefore ramp over roughly a quarter rather than switch
on, and the paper should not describe activation as a switch.

\textbf{Nor should the activation announcement.} These four figures belong in it verbatim, before
activation rather than after. An operator who has been told that \PNR{} pays \SI{20}{\percent} of
application revenue, and who observes \SI{3.3}{\percent} of it on day one, will reasonably conclude
the mechanism is broken; the same operator told in advance that the pool fills on a thirty-day time
constant will read the same observation as confirmation that it works. This is the one place in
\PNR{}'s deployment where the difference between a successful launch and a credibility problem is a
communications decision rather than a design one, and it is recorded here so that it is nobody's
oversight.
...
in this paper \srcintent{design intake} (C24). What \PNR income \emph{is} exactly
...
The last row is not a hypothetical, and the review trigger recommended below should be read in that
light. The fleet has been substantially larger than it is now, and Cumulus is the tier that grows
first when it grows, because it is the cheapest to enter. \textbf{A return to a fleet size this
network has already sustained would fire the $2\times$ trigger}, so the trigger should be treated as
a scheduled review of a constant that is fixed by consensus --- and therefore revisable only by
network upgrade --- rather than as a remote contingency. Recording that now costs nothing; discovering
it at $2\times$ costs a hard fork under time pressure.
...
\srcintent{design intake}. The trade-off it resolves does not disappear with
...
% ---------------------------------------------------------------------------
\subsection{What \PNR{} does and does not incentivise}
\label{sec:pnr:hosting}

This is the sharpest question about the design and it is answered here rather than buried. It is
also the one most easily stated wrongly, so the distinction comes first.

\textbf{\PNR{} is a strong incentive to \emph{operate a node}.} It is the mechanism by which
application revenue reaches operators at all, and on the team's own projection it becomes the largest
component of operator income \srcintent{design intake}. Nothing below qualifies that.

What the property proves is narrower: income does not vary with \emph{which} applications a
particular node happens to run. That is not a free-riding opportunity, because \textbf{an operator
does not choose what to host}. \FluxOS{}'s spawner selects a candidate at random from the
network-wide application list \shipped
\srccode{flux/\allowbreak{}ZelBack/\allowbreak{}src/\allowbreak{}services/\allowbreak{}appLifecycle/\allowbreak{}appSpawner.js}{311},
and no opt-out, allowlist or per-application refusal exists anywhere in the deployed orchestration
code. Placement is the network's decision, not the operator's. An operator who wanted to collect
\PNR{} while hosting nothing could not simply decline work; they would have to appear eligible while
providing no real capacity --- which is a statement about the \emph{eligibility gate}, not about the
reward.

\paragraph{What that does and does not imply for sequencing, at the right magnitude.} It is
tempting, and this paper was tempted, to conclude that hardening attestation is a
\emph{precondition} for \PNR{} because \PNR{} raises the payoff to passing the gate without doing the
work by exactly what it pays. The first half is true; the inference is not, and the arithmetic is in
this section already. At activation \PNR{} would pay $\num{286320}\flux$ per year against
$\num{6386040}\flux$ per year of issuance already paid through \emph{the same unbound gate}
(\S\ref{sec:pnr:crossover}). \PNR{} would raise the prize for faking eligibility by
\SI{4.5}{\percent}. \textbf{The gate problem is a \PoN{} problem today, and \SI{95.7}{\percent} of it
exists whether or not \PNR{} ever ships.}

Two conclusions follow, and the second is the one this paper now carries.
\begin{enumerate}
  \item Hardening the eligibility gate is the network's most urgent security work \emph{on its own
  merits}, independent of \PNR{}. Framing it as a \PNR{} precondition understates it by more than an
  order of magnitude and invites the reply that a \SI{4.3}{\percent} supplement is not worth blocking.
  \item The defensible sequencing rule is therefore not ``\PNR{} waits'' but \textbf{\PNR{}'s ramp
  tracks attestation's completion}: activation at $\nodeshare = 0.20$ adds little to an exposure that
  already exists, while each step of the ramp toward $\nodesharecap = 0.50$ moves a larger share of
  operator income behind a gate whose strength should have risen to meet it. This is a schedule
  coupling, not a blocking dependency, and it does not hand an opponent of \PNR{} a principled veto.
\end{enumerate}
\Cref{sec:feasibility} and \Cref{sec:migration} state the same edge in the same terms. What makes the
coupling achievable rather than aspirational is the finding in
\S\ref{sec:boot-chain}: the image-bound PCR policy the hardening needs is already generated on every
capable machine, and what is missing is a key bound to it and a verifier that reads it.
...
which, given that placement is not the operator's to refuse, is a claim about how cheaply a node can
satisfy the eligibility gate, not about whether it will accept the work it is given.
\end{property}
...
parity-scale revenue ($22\times$) and at the cap, $0.06$--$1.0\flux$ per year. \textbf{The collective
interest in the network hosting well is real; the private share of it is negligible at every
parameter value in range.} This is not a tuning problem; it is structural to the settled decision to
pay all nodes.

What that does \emph{not} say --- and an earlier draft of this remark did say --- is that free-riding
dominates. Free-riding would require the ability to decline the work, and no such ability exists
(\S\ref{sec:pnr:hosting}). The correct reading is narrower and points somewhere useful: \PNR{}
supplies no incentive gradient that rewards hosting \emph{well}, so what protects hosting quality is
entirely the eligibility gate and the \num{640}-block re-confirmation that removes nodes failing it
\shipped \srccode{fluxd/\allowbreak{}src/\allowbreak{}fluxnode/\allowbreak{}fluxnode.cpp}{790--820} --- which is
the same conclusion the sequencing argument above reaches, from the other side.
\end{remark}
...
installation'' \srcintent{design intake} (\planned). That reasoning is sound and
...
oversight. What the choice does not do is remove the cost: the whole hosting guarantee is transferred
onto the eligibility gate. A node is a \PoN payee, and therefore a \PNR payee, only if it is confirmed
with a \fluxbench-signed transaction and, in the target architecture, attested (\S14). \PNR's hosting
...
pays} --- \SI{4.29}{\percent} of operator income at activation (\S\ref{sec:pnr:crossover}), more as
the ramp advances (C24).

The complementary figure is what turns that observation into a schedule rather than a veto, and it is
stated here so the two never travel apart: the remaining \SI{95.71}{\percent} of operator income at
activation is \emph{issuance already paid through the same gate}. The gate problem is not created by
\PNR{} and would not be removed by cancelling it. \S4 and \S14 establish the gate's properties;
\S\ref{sec:feasibility} classes the remaining attestation work as engineering once it is scoped to the
TPM population; and \S\ref{sec:migration} couples \PNR's \emph{ramp} to that work rather than blocking
activation on a bonded network that is neither necessary for the fix nor deliverable by
\Cref{tab:pnr:activation}'s first row. Attested execution is not adjacent to \PNR; it is what would
make \PNR pay for something, and it is what already makes issuance pay for something.
\end{remark}
...
its data is deleted, and there is no refund and no residue \srcintent{design intake}. \textbf{Moderation eviction} is different, because it interrupts a period that has been paid for
...
forfeited rather than refunded \srcintent{design intake}. The question is where that
value goes.
...
\srcintent{design intake}. $\pool$ is excluded by S2, and refund by the settled
...
\textbf{PA Depletion} ($\times 1/2$)  & \textbf{3{,}787{,}502} & \textbf{2027-07-01} & $\mathbf{6.3}$ & $\mathbf{0.20}$ --- \PNR begins\\
...
\srcintent{design intake}. Dates are $\SI{30}{\second}$ extrapolations from the \PoN
...
$\hpa = \num{3787502}$ \srcintent{design intake} (\planned), superseding the
...
\srcintent{design intake}. \textbf{Parity depends on demand growth, not on
price.}
...
\srcintent{design intake}. The generated data carries both bases
...
\midrule
3{,}787{,}502  & 2027-07-01 & $6.3000$ & $0.20$ & 6{,}386{,}040 & 31{,}930{,}200 & ---\\
...
requirement falls from $31.9$M to $6.8$M\flux\ per year in $5.3$ years and to $1.7$M on the
...
\SI{10}{\percent}  & 2038-10 & $4.21$M & $0.50$\\
\SI{25}{\percent}  & 2034-10 & $7.33$M & $0.50$\\
\SI{50}{\percent}  & 2032-10 & $12.4$M & $0.50$\\
\SI{100}{\percent} & 2030-10 & $14.3$M & $0.40$\\
\SI{200}{\percent} & 2029-10 & $18.3$M & $0.35$\\
\bottomrule
\end{tabular}
\caption{Crossover year under constant annual revenue growth from $V_0$, \flux terms \planned.
Required sustained growth to reach parity by a given year: 2029, \SI{162}{\percent};
2030, \SI{82}{\percent}; 2031, \SI{51}{\percent}; 2032, \SI{34}{\percent}; 2033, \SI{26}{\percent};
...
allocation for other node ops'' \srcintent{design intake} (\planned). The team
...
will be more decentralized'' \srcintent{design intake}. But the stated response to
...
\begin{tikzpicture}
\begin{axis}[
  width=0.74\linewidth, height=6.4cm,
  xlabel={year},
  ylabel={\flux\ per year (log scale)},
  ymode=log,
  xmin=2027, xmax=2046,
  %% Outside the axes: the g=100% series climbs straight through a north-west legend.
...
output of \texttt{scripts/\allowbreak{}07-pnr-settlement.\allowbreak{}py} (run 2026-09-07), which also wrote
...
\toprule
 & \multicolumn{5}{c}{exogenous price multiple $m$}\\
\cmidrule(l){2-6}
real growth $g$ & $0.5$ & $1$ & $2$ & $5$ & $10$\\
\midrule
\SI{0}{\percent}   & $14.3$ & never & never & never & never\\
\SI{10}{\percent}  & $7.3$  & $11.3$ & $14.3$ & $19.3$ & $26.1$\\
\SI{25}{\percent}  & $5.3$  & $7.3$  & $9.3$  & $12.3$ & $14.3$\\
\SI{50}{\percent}  & $3.3$  & $5.3$  & $6.3$  & $7.3$  & $9.3$\\
\SI{100}{\percent} & $2.3$  & $3.3$  & $4.3$  & $5.3$  & $6.3$\\
\bottomrule
\end{tabular}
...
month''. Domain: $\{\text{trailing receipts at the payment sink}$,
...
\midrule
O1 & drip constant $\drip$ & $86{,}400$ blocks, i.e.\ exactly \SI{30}{\day} at \SI{30}{\second} & \srcintent{design intake}\\
O2 & commitment of $\pool_\hgt$ & committed in the coinbase (PC) and recomputed by every validator; no separate store, no header field & \srcintent{design intake}, \srcdoc{reconciliation record}\\
O3 & memo semantics & \texttt{OP\_RETURN} binding $H(m_\app)$, carried and not interpreted by consensus (PS2) & \srcintent{design intake}\\
O18 & serialisation and enforcement boundary & one output of the full price to $\mathsf{sink}$ (PS1); consensus applies the split (PS4); \FluxOS checks only existence, memo and coverage & \srcintent{design intake}, \srcdoc{reconciliation record}\\
\bottomrule
\end{tabular}
...
O9 & price-table rebasing & discretionary; rule-based against a real-terms anchor; receipts-referencing (withdrawn as circular) & revisions are already on-chain and dated; a real-terms rule needs the price feed the quoting path already runs\\
...
O14 and O19 remain open. The bold row is the one that gates the rest, and it is mirrored in the
...
\srcintent{design intake}, and several are simply a
...
author} \srcintent{design intake}. \planned, \srcinferred{} for the derivations.}
...
(\Cref{rem:o7-moot}) \\
O9 price rebasing & \textbf{rule-based, real-terms anchor} & a receipts-referencing rule is
circular; the price feed it would need is already running off-consensus, with the on-chain table as a
floor \\
...
\textbf{O9, price-table rebasing --- and a circularity the first draft of this recommendation did not
see.} The price table is a lever over pool inflow, which sits awkwardly beside a mechanism whose
legitimacy rests on operators being paid from receipts rather than from discretion. Two corrections
have to be made before the recommendation can stand.

\emph{First, the lever is narrower than ``Foundation policy'' suggests.} A price revision is not a
configuration edit: it is an on-chain message, sent from a Foundation multisig address to itself,
carrying the seven-field table \texttt{p\_cpu\_ram\_hdd\_minPrice\_port\_scope\_staticip}, which every
node parses from its own copy of the chain and applies from the message's height onward \shipped
\srccode{flux/\allowbreak{}ZelBack/\allowbreak{}src/\allowbreak{}services/\allowbreak{}explorerService.js}{279--303,387},
\srccode{flux/\allowbreak{}ZelBack/\allowbreak{}src/\allowbreak{}services/\allowbreak{}utils/\allowbreak{}chainUtilities.js}{9--52}.
The authority is one multisig; the \emph{exercise} of it is public, dated, ordered and
non-repudiable, and every node computes prices from the same on-chain history. That is a materially
better starting point than a table published over an API, and the paper should credit it.

\emph{Second, ``a published function of trailing measured receipts'' is circular.} Receipts are price
times volume. A rule that sets next period's price from last period's receipts has no exogenous
anchor: it is a feedback loop that can drift arbitrarily far from the cost of the resources being
sold, in either direction, and calling it a rule rather than a lever changes who turns the crank, not
whether the number means anything. What the deployed table has actually tracked across its five
revisions --- a \SI{100}{\percent}-fold fall in the CPU rate --- is real cost and the \flux{} price,
not receipts.

So the choice is a real one and it should be stated as such. \emph{Either} the rule targets a
real-terms anchor, in which case it needs a price feed, \emph{or} it stays discretionary and the paper
says so. What it cannot be is a rule that appears to remove discretion while quietly reintroducing it.

On the first branch, one fact changes the cost of the option: \textbf{a price feed already exists in
the deployed system}, and this paper's earlier framing of an oracle as a new dependency was wrong.
The quoting path fetches a live \flux/USD rate from \texttt{viprates.\allowbreak{}runonflux.\allowbreak{}io}, falls
back to CoinGecko, and falls back again to a compiled-in constant; it prices a USD-denominated table,
applies duration discounts and a \flux{}-payment discount, and returns the \emph{larger} of that
figure and the on-chain \flux{} price \shipped
\srccode{flux/\allowbreak{}ZelBack/\allowbreak{}src/\allowbreak{}services/\allowbreak{}utils/\allowbreak{}appSpecHelpers.js}{440--522}. The
on-chain table is thus already a floor beneath a USD-anchored quote. A real-terms rebasing rule would
not import a trust dependency the network lacks; it would move one it already runs into a place where
its failure modes have to be specified --- which is a smaller step than this section previously
claimed, and a more honest description of the status quo.

\emph{Recommendation, revised.} Adopt a rule that rebases against a real-terms anchor, and state the
feed and its failure behaviour as part of the rule rather than as an implementation detail; the
consensus-side floor already in place bounds what a bad feed can do to receipts. If that is judged too
much trust to formalise, keep the table discretionary and say so plainly. The one option this paper
withdraws is the receipts-referencing rule, which is circular.
