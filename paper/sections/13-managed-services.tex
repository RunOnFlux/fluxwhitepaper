...
\begin{tikzpicture}[
  ...
  %% Above the shaded rect's top edge at y=40 -- inside it, the boundary struck
  %% through the text.
  \node[lbl, anchor=west] at (74,41.6) {follower receipt: not awaited, not bounded};
  ...
  %% The collapsed loss window is drawn as a zero-width bar. It must be drawn BEFORE
  %% the event dot and the label, or it paints over both -- which is what it did.
  \draw[very thick] (58,3.6) -- (58,6.4);
  \node[ev] (r3)  at (58,5) {}; \node[lbl, below=0.5mm of r3] {replica\\acknowledges};
  \node[lbl] at (58,7.3) {\textbf{ack}};
  %% Same as the first timeline: 0.6 above a rule at y=5 still crosses the glyphs.
  \node[lbl, anchor=west] at (67,7.3) {loss window collapses to zero};
...
other node APIs sold as a product \srcintent{design intake} (\planned).
